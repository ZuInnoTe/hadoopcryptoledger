...
public class BitcoinRawBlockRecordReader implements RecordReader<BytesWritable, BytesWritable> {
...
/**
* Creates a Record Reader for raw Bitcoin blocks represented as BytesWriteable, ie each raw Bitcoin block is a record.
...
public boolean next(BytesWritable key, BytesWritable value) throws IOException {
	...
	while(getFilePosition()<=end) { // did we already went beyond the split (remote) or do we have no further data left?
		...
		try {
			dataBlock=bbr.readRawBlock();
		} catch (BitcoinBlockReadException e) {
			// log
			LOG.error(e);
		}	
		if (dataBlock==null) return false;
		byte newKey[]=bbr.getKeyFromRawBlock(dataBlock);
