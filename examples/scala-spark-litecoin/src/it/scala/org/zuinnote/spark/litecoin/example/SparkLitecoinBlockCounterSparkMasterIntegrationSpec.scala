...
package org.zuinnote.spark.litecoin.example
...
class SparkLitecoinBlockCounterSparkMasterIntegrationSpec extends AnyFlatSpec with BeforeAndAfterAll with GivenWhenThen with Matchers {
 
private var sc: SparkContext = _
private val master: String = "local[2]"
private val appName: String = "example-scalasparklitecoinblockcounter-integrationtest"
...
"The genesis block on DFS" should "be 1 transactions i total" in {
	...
	val classLoader = getClass().getClassLoader()
    	// put testdata on DFS
    	val fileName: String="litecoingenesis.blk"
	...
	Given("Configuration")
	 conf.set("hadoopcryptoledger.bitcoinblockinputformat.filter.magic","FBC0B6DB")
	When("count total transactions")
	SparkScalaLitecoinBlockCounter.jobTotalNumOfTransactions(sc,conf,dfsCluster.getFileSystem().getUri().toString()+DFS_INPUT_DIR_NAME,dfsCluster.getFileSystem().getUri().toString()+DFS_OUTPUT_DIR_NAME)
