...
private val appName: String = "example-scalasparkbitcoinblockds-integrationtest"
...
"The genesis block on DFS" should "have 5000000000 of summarized transaction outputs" in {
	...
	When("sum outputs of all transactions of genesis block")
	SparkScalaBitcoinBlockDataSource.sumTransactionOutputsJob(sqlContext,dfsCluster.getFileSystem().getUri().toString()+DFS_INPUT_DIR_NAME,dfsCluster.getFileSystem().getUri().toString()+DFS_OUTPUT_DIR_NAME)
	Then("5000000000 in total as a result")
	// fetch results
	val resultLines = readDefaultResults(1)
	assert(1==resultLines.size())
	assert("[5000000000]"==resultLines.get(0))
}
