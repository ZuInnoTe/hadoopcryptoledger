...
private val appName: String = "example-scalasparkethereumblockds-integrationtest"
...
"The block 1346406 on DFS" should "have 5000000000 of summarized transaction outputs" in {
	...
	When("sum outputs of all transactions of block 1346406")
	SparkScalaEthereumBlockDataSource.sumTransactionOutputsJob(sqlContext,dfsCluster.getFileSystem().getUri().toString()+DFS_INPUT_DIR_NAME,dfsCluster.getFileSystem().getUri().toString()+DFS_OUTPUT_DIR_NAME)
	Then("6610526380000000000 in total as a result")
	// fetch results
	val resultLines = readDefaultResults(1)
	assert(1==resultLines.size())
	assert("[6610526380000000000]"==resultLines.get(0))
}
