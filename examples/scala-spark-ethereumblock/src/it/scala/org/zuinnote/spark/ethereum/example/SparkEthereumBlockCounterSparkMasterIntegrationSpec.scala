...
class SparkEthereumBlockCounterSparkMasterIntegrationSpec extends AnyFlatSpec with BeforeAndAfterAll with GivenWhenThen with Matchers {
 
private var sc: SparkContext = _
private val master: String = "local[2]"
private val appName: String = "example-scalasparkethereumblockcounter-integrationtest"
...
"The block 1346406 on DFS" should "be 6 transactions in total" in {
	...
	SparkScalaEthereumBlockCounter.jobTotalNumOfTransactions(sc,conf,dfsCluster.getFileSystem().getUri().toString()+DFS_INPUT_DIR_NAME,dfsCluster.getFileSystem().getUri().toString()+DFS_OUTPUT_DIR_NAME)
	Then("6 transaction in total as a result")
	// fetch results
	val resultLines = readDefaultResults(1)
	assert(1==resultLines.size())
	assert("(No of transactions: ,6)"==resultLines.get(0))
}
